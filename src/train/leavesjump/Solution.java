...
public class Solution {
   ...
   * 
   * For example, you are given integer X = 5 and array A such that: A[0] = 1 A[1]
   * = 3 A[2] = 1 A[3] = 4 A[4] = 2 A[5] = 3 A[6] = 5 A[7] = 4
   * 
  ...
  public int solution(int X, int[] A) {
    int time = 0, progress = 0;

    // initialize an array that keeps track of the continuous path available over
    // the river
    int[] path = new int[X];
    for (int i = 0; i < X; i++) {
      path[i] = 0;
    ...
    return progress == X ? time : -1;
  }

  public static void main(String[] args) {
    var sol = new Solution();

    System.out.println(sol.solution(5, new int[] { 1, 3, 1, 4, 2, 3, 5, 4 })); // 6
    System.out.println(sol.solution(5, new int[] { 2, 3, 4 })); // -1
    System.out.println(sol.solution(5, new int[] { 1, 1, 1, 1, 1 })); // -1
    System.out.println(sol.solution(5, new int[] { 1, 2, 3, 4, 5 })); // 5
    System.out.println(sol.solution(5, new int[] { 5, 4, 3, 2, 1 })); // 5
  }
